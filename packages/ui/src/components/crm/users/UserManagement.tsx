...
import { usersApi, userOrganizationsApi, organizationsApi } from "../../../lib/api";
...
export function UserManagement() {
  ...
  const { data: userOrgs } = useQuery({
    ...
    queryFn: userOrganizationsApi.getAll,
  });
  ...
  const getOrganizationName = (userId: string): string => {
    const userOrg = userOrgs?.data?.find((uo: UserOrganization) => uo.userId === userId);
    if (!userOrg) return "No Organization";
    
    const org = organizations?.data?.find((o: Organization) => o.id === userOrg.orgId);
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {user.sso && (
                      <Badge variant="outline" className="text-xs bg-primary-subtle border-primary/20 text-primary">
                        SSO
                      </Badge>
                    )}
                  </div>
                  <p className="text-sm text-muted-foreground truncate">{user.email}</p>
                  <p className="text-xs text-muted-foreground mt-1">{getOrganizationName(user.id)}</p>
                </div>
                
                <div className="flex items-center gap-3">
                  <Badge variant="outline" className={`text-xs ${statusColors[user.status || 'active']}`}>
                    {user.status || 'active'}
                  </Badge>
